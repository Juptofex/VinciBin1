...
public class Vol {
	...
	public Coordonnees niemeLieu(int n){
		if (n < 0) return null;
		if (n > tableCoordonnees.length-1) return null;
	...
	public Coordonnees lieuLePlusAuSud() {
		long pSud = tableCoordonnees[0].getLatitude();
		int indice = 0;
		int i = 0;
		for (Coordonnees tableCoordonnee : tableCoordonnees) {
			if (tableCoordonnee.getLatitude() < pSud) {
				pSud = tableCoordonnee.getLatitude();
				indice = i;
			}
			i++;
		}
		return tableCoordonnees[indice];
	}
	...
	public Coordonnees lieuLePlusEloigne(){
		double pLoin = tableCoordonnees[0].distance(tableCoordonnees[0]);
		int indice = 0;
		int i = 0;
		for (Coordonnees tableCoordonnee : tableCoordonnees) {
			if (tableCoordonnees[0].distance(tableCoordonnee) > pLoin) {
				pLoin = tableCoordonnees[0].distance(tableCoordonnee);
				indice = i;
			}
			i++;
		}
	...
	public boolean cibleAtteinte(Coordonnees cible){
		...
		for (Coordonnees tableCoordonnee : tableCoordonnees) {
			if (tableCoordonnee.equals(cible)) return true;
		}
		return false;
	}
	...
	public double distance() {
		...
		for (Coordonnees tableCoordonnee : tableCoordonnees) {
			if (x != 0) {
				dist += tableCoordonnee.distance(tableCoordonnees[x-1]);
			}
			x++;
		}
		return dist;
