...
public class SerieEtudiants {
    ...
    public boolean insererApresDelegue(String nouvelEtudiant){
        ...
        if (mapEtudiantNoeud.containsKey(nouvelEtudiant))
            return false;
        Noeud noeud = new Noeud(nouvelEtudiant);
        ...
        mapEtudiantNoeud.put(nouvelEtudiant, noeud);
        return true;
    }
    ...
    public boolean tronquerAPartir(String etudiant){
        if(etudiant==null||etudiant.length()==0)
            throw new IllegalArgumentException();
        //TODO
        if (etudiant.equals(tete.etudiant) || !mapEtudiantNoeud.containsKey(etudiant))
            return false;
        Noeud noeud = mapEtudiantNoeud.get(etudiant);
        Noeud temp = noeud;
        while (temp!=null) {
            mapEtudiantNoeud.remove(temp.etudiant);
            temp=temp.suivant;
        }
