...
public class FileDePrioriteImpl<E> implements FileDePriorite<E>{
	...
	public void insere(E e) {
		...
		racine = insereRecursif(racine, e);
	}

	private Noeud insereRecursif(Noeud n, E e) {
		if (n==null)
			return new  Noeud(e);
		if (comparateur.compare(n.element, e)<0) {
			n.gauche = insereRecursif(n.gauche, e);
		} else {
			n.droit = insereRecursif(n.droit, e);
		}
		return n;
	}
	...
	private E maxRecursif(Noeud n) {
		if (n.gauche==null)
			return n.element;
		return maxRecursif(n.gauche);
	}
