...
public class FileDePrioriteImpl<E> implements FileDePriorite<E>{
	...
	public void insere(E e) {
		...
		racine = insereRecursivement(racine, e);
	}

	private Noeud insereRecursivement(Noeud noeud, E e) {
		if (noeud==null) {
			return new Noeud(e);
		}
		if (comparateur.compare(noeud.element, e)<0) {
			noeud.droit = insereRecursivement(noeud.droit, e);
		} else {
			noeud.gauche = insereRecursivement(noeud.gauche, e);
		}
		return noeud;
	}
	...
	public E max() {
		...
		if (racine==null)
			return null;
		return maxRecursif(racine);
	}

	private E maxRecursif(Noeud noeud) {
		if (noeud.droit==null)
			return noeud.element;
		return maxRecursif(noeud.droit);
	}
