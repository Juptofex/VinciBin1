...
public class Equipe {
    ...
    public boolean contient (String equipier){
        ...
        //TODO
        for (String membre: mapEquipierNoeud.keySet()) {
            if (membre==equipier)
                return true;
        }
        return false;
    ...
    public boolean ajouterEnFin(String nouvelEquipier){
        ...
        if (contient(nouvelEquipier))
            return false;
        Noeud noeud = new Noeud(queue.precedent,nouvelEquipier, queue);
    ...
    public boolean tronquerApres(String equipier){
        if(equipier==null||equipier.length()==0)
            throw new IllegalArgumentException();
        //TODO
        if (!contient(equipier) || queue.precedent.equipier.equals(equipier))
            return false;
        Noeud noeud = mapEquipierNoeud.get(equipier);
        Noeud temp = noeud.suivant;
        while (temp!= null && temp != queue) {
            mapEquipierNoeud.remove(temp.equipier);
            temp=temp.suivant;
        }
        noeud.suivant=queue;
        queue.precedent=noeud;
